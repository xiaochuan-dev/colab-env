...
def train_one_epoch(model, loader, optimizer, scaler, criterion, vocab, device, epoch):
    ...
    for imgs, tgt, _ in pbar:
        ...
        with autocast(enabled=cfg.USE_AMP):
            ...
            # logits: (B, T-1, V)  vs tgt[:, 1:]
            loss = criterion(logits.reshape(-1, logits.size(-1)), tgt[:, 1:].reshape(-1))

            # 可选 Spatial Grounding Loss（im2latexv2 默认 λ=0）
            if cfg.LAMBDA_SG > 0 and mus:
                # 没有真实 bbox 时跳过；有则在这里加 SmoothL1
                pass
...
@torch.no_grad()
def evaluate(model, loader, criterion, vocab, device, epoch):
    model.eval()
    total_loss = 0.0
    all_preds = []
    all_gts = []
    ...
    exp_rate = compute_exp_rate(all_preds, all_gts, vocab)
...
def main():
    ...
    for epoch in range(1, cfg.NUM_EPOCHS + 1):
        t0 = time.time()
        train_loss = train_one_epoch(model, train_loader, optimizer, scaler, criterion, vocab, device, epoch)
        scheduler.step()

        if epoch % cfg.EVAL_EVERY == 0:
            val_loss, exp_rate = evaluate(model, val_loader, criterion, vocab, device, epoch)
            print(f"Epoch {epoch}: train_loss={train_loss:.4f}  val_loss={val_loss:.4f}  "
                  f"ExpRate={exp_rate*100:.2f}%  lr={optimizer.param_groups[0]['lr']:.6f}  "
